...
def forward_diffusion_sample(x_0, t, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod, xy1, xy2, device=device):
    """
...
model = SimpleUnet(DIVIDER=MODEL_DIVIDER).to(device)
...
x_noisy = forward_diffusion_sample(input_tensor, t, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod,
                                   (box[0], box[1]), (box[2], box[3]), device=device)
